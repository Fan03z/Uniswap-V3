...
// FIXME: 直接导入 import "../../contracts/lib/PoolAddress.sol";报错:
// Error:
// Compiler run failed
// error[2333]: DeclarationError: Identifier already declared.
//  --> test/foundry/Assertions.sol:7:1:
//   |
// 7 | import "../../contracts/UniswapV3NFTManager.sol";
//   | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// Note: The previous declaration is here:
//  --> contracts/lib/Math.sol:7:1:
//   |
// 7 | library Math {
//   | ^ (Relevant source part starts here and spans across multiple lines).
import {UniswapV3NFTManager} from "../../contracts/UniswapV3NFTManager.sol";
...
abstract contract Assertions is Test {
  ...
  function assertBalances(ExpectedBalances memory expected) internal {
    ...
    assertEq(expected.tokens[0].balanceOf(address(expected.pool)), expected.poolBalance0, "incorrect token0 balance of pool");
    assertEq(expected.tokens[1].balanceOf(address(expected.pool)), expected.poolBalance1, "incorrect token1 balance of pool");
  }
  ...
  function assertTick(ExpectedTick memory expected) internal {
    ...
    // assertEq(tickInBitMap(expected.pool, expected.tick), expected.initialized, "incorrect tick in bitmap state");
  }
  ...
  function assertPoolAndBalances(ExpectedPoolAndBalances memory expected) internal {
    assertPoolState(
      ExpectedPoolState({
        pool: expected.pool,
        liquidity: expected.liquidity,
  ...
  function assertPosition(ExpectedPosition memory params) internal {
