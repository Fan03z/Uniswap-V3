...
//   |
// 9 | import "./lib/PoolAddress.sol";
//   | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// Note: The previous declaration is here:
//  --> @openzeppelin/contracts/utils/math/Math.sol:9:1:
//   |
// 9 | library Math {
//   | ^ (Relevant source part starts here and spans across multiple lines).
import {PoolAddress as PoolAddress} from "./lib/PoolAddress.sol";
import "./lib/TickMath.sol";

contract UniswapV3NFTManager is ERC721 {
  // 操作用户不是NFT所有者或者未得到转移授权
  error NotAuthorized();
  // 滑点保护
  ...
  modifier isApprovedOrOwner(uint256 tokenId) {
    ...
    if (msg.sender != owner && !isApprovedForAll[owner][msg.sender] && getApproved[tokenId] != msg.sender) revert NotAuthorized();
  ...
  function addLiquidity(AddLiquidityParams calldata params) public returns (uint128 liquidity, uint256 amount0, uint256 amount1) {
  ...
  ) public isApprovedOrOwner(params.tokenId) returns (uint128 amount0, uint128 amount1) {
    TokenPosition memory tokenPosition = positions[params.tokenId];
    if (tokenPosition.pool == address(0x00)) revert WrongToken();
  ...
  function _addLiquidity(
    ...
    (amount0, amount1) = params.pool.mint(
      ...
      params.upperTick,
      liquidity,
      abi.encode(IUniswapV3Pool.CallbackData({token0: params.pool.token0(), token1: params.pool.token1(), payer: msg.sender}))
    );
