...
library Oracle {
  ...
  function transform(Observation memory last, uint32 timestamp, int24 tick) internal pure returns (Observation memory) {
    ...
    return
      Observation({timestamp: timestamp, tickCumulative: last.tickCumulative + int56(tick) * int56(delta), initialized: true});
  }
  ...
  function observeSingle(
    ...
    if (secondsAgo == 0) {
      Observation memory last = self[index];
      if (last.timestamp != time) transform(last, time, tick);
